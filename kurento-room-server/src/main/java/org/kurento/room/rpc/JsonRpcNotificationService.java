...
public class JsonRpcNotificationService implements UserNotificationService {
	...
	private static ConcurrentMap<String, SessionWrapper> sessionsByMobileNumber = new ConcurrentHashMap<String, SessionWrapper>();
	...
	public SessionWrapper addTransactionForSessionByMobileNumber(Transaction t, Request<JsonObject> request) {
		// get the session id by transaction id
		String sessionId = t.getSession().getSessionId();
		SessionWrapper sw = sessionsByMobileNumber.get(request.getParams().get("from").getAsString());
		String mobileNumber = request.getParams().get("from").getAsString();

		if (sw == null) {
			//first time register
			sw = new SessionWrapper(t.getSession());
			SessionWrapper oldSw = sessionsByMobileNumber.putIfAbsent(mobileNumber, sw);
			if (oldSw != null) {
				log.warn("Concurrent initialization of session wrapper #{}", sessionId);
				sw = oldSw;
			}
		} else {
			//not first time but reconnecting
			//get the session wrapper for existing number
			sw = new SessionWrapper(t.getSession());
			SessionWrapper oldSw = sessionsByMobileNumber.get(mobileNumber);
			String roomId = request.getParams().get("roomId").getAsString();
			if (roomId != null && !roomId.isEmpty()) {
				log.warn("Concurrent initialization of session wrapper #{}", sessionId);
				SessionWrapper newSw = sessionsByMobileNumber.put(mobileNumber, sw);
	...
	public void sendNotification(final String participantId, final String method, final Object params) {
		...
		if (sw == null || sw.getSession() == null) {
			log.error("No session found for id {}, unable to send notification {}: {}", participantId, method, params);
			return;
		}
	...
	public static ConcurrentMap<String, SessionWrapper> getSessionsByMobileNumber() {
		return sessionsByMobileNumber;
	}

	public static void setSessionsByMobileNumber(ConcurrentMap<String, SessionWrapper> sessionsByMobileNumber) {
		JsonRpcNotificationService.sessionsByMobileNumber = sessionsByMobileNumber;
	}
