...
public class JsonRpcUserControl {
  ...
  @Autowired
  private UserNotificationService notifyService;
  ...
  public void callUser(Transaction transaction, Request<JsonObject> request) {
	  JsonObject json = new JsonObject();
	  request.getParams().get("testname");
	  
	  System.out.println("Requested from ==>> "+request.getParams().get("from").getAsString());
	  System.out.println("Requested to ==>> "+request.getParams().get("to").getAsString());
	  //generate the room id
	  String roomId = UUID.randomUUID().toString();
	  json.addProperty("roomId", roomId);
	  //send the room number to "from" user
	  notifyService.sendNotification(transaction.getSession().getSessionId(), request.getMethod(), json);
	  //send the room number to "to" user
	  String to = request.getParams().get("to").getAsString();
	  //check if the number has a session generated
	  if(JsonRpcNotificationService.getSessionsByMobileNumber().get(to) != null) {
		  String participantId = JsonRpcNotificationService.getSessionsByMobileNumber().get(to).getSession().getSessionId();
		  notifyService.sendNotification(participantId, request.getMethod(), json);
	  } else {
		  throw new RoomException(Code.USER_NOT_FOUND_ERROR_CODE , "Cannot call. As the user is not found");
	  }
  }
}
